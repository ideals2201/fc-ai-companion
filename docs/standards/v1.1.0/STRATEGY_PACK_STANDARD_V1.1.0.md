...
Every Strategy Pack must include `strategyDescriptions` in `manifest.json`.

`strategyDescriptions` is the standard explanation catalog for the strategy categories included in the package. It is what the UI should show when the user loads a package and wants to understand what each strategy is meant to do before looking at the numbers.

Rules:

- Every key in `strategySlots` must have one `strategyDescriptions[<strategyKey>]` entry.
- Every key in `strategyKeys` must also be present in `strategyDescriptions`.
- Descriptions must use English as the internal canonical text. Localized UI text is optional and must live under `localization`.
- Descriptions must not claim validation. Use `quality.strategyResults` and `training-progress.json` for factual status.
- A disabled or untrained strategy still needs a description. Its data status should be `unstarted` or `unverified`, not omitted.
- Stage-specific performance must not be stored only in the description. It belongs in each stage's `training-progress.json`.
...
```json
{
  "strategyDescriptions": {
    "survival-v0": {
      "label": "Robust Survival",
      "summary": "Prioritizes survival, slows progression when needed, and clears required fixed threats.",
      "playStyle": "Suitable for single-AI clear attempts, human-AI companion play, and dual-AI stability baselines.",
      "riskPolicy": "Safety override takes priority over speed, rewards, and kill count.",
      "dataSource": "manifest.strategyDescriptions + stages/<stage-id>/training-progress.json",
      "localization": {
        "zh-CN": {
          "label": "????",
          "summary": "?????????????????????????",
...
The cockpit should present `strategyDescriptions` together with `quality.strategyResults` and each stage ledger:

- `strategyDescriptions`: what this strategy type means.
- `quality.strategyResults`: compact package-level battle summary.
- `stages/<stage-id>/training-progress.json`: factual per-stage progress, deaths, time, clear progress, and evidence.
